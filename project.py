...
def newItem(category_name):
    ...
    if request.method == 'POST':
        newlistItem = Item(name=request.form['name'], description=request.form['description'], category_name=category_name, user_id=category.user_id)
        session.add(newlistItem)
        session.commit()
        flash('New Item %s Successfully Created' % (newlistItem.name))
